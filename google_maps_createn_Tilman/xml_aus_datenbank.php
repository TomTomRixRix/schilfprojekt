...
while ($row = mysql_fetch_assoc($resultmarker)){ 
    ...
    echo 'imgurl="'. parseToXML($row['bildlink']) . '" ';
	echo 'lat="' . $row['lat'] . '" ';
	echo 'lng="' . $row['lng'] . '"';
	echo '/>';	
}
...
$querypolygone = 'SELECT * FROM Koordinaten WHERE Art = "polygone" GROUP BY Name';
...
$querypolyline = 'SELECT * FROM Koordinaten WHERE Art = "polyline" GROUP BY Name';
...
while ($row = mysql_fetch_assoc($resultpolyline)){
	...
	echo '<polyline ';
	echo 'name="' . parseToXML($row['Name']) . '" ';
	echo 'pflanze="' . parseToXML($row['Pflanze']) . '" ';
    echo 'imgurl="'. parseToXML($row['bildlink']) . '"> ';
