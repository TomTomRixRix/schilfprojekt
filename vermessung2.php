...
<div id="resultsContent" class="registerContent">
	...
	if($orderby=="1"){
	    ...
	    $anfrage = 'SELECT StandortID,Standort,Datum, ROUND(AVG(LaengeUnterWasser),2),ROUND(AVG(LaengeUeberWasser),2),ROUND(AVG(LaengeGesamt),2),ROUND(AVG(DickeUnten),2), ROUND(AVG(DickeMitte),2),
					ROUND(AVG(Blattbreite),2),ROUND(AVG(Blattlaenge),2), ROUND(AVG(Blattzahl),2), ROUND(AVG(Rispenlaenge),2),ROUND(AVG(Nodienzahl),2)
					FROM biologischeEinzelmessergebnisse
					GROUP BY Standort, Datum
